...
if exists('g:loaded_complexity')
    finish
endif

let g:loaded_complexity = 1
...
class ASTComplexity(ASTVisitor):
 ...
 node):
        ...
        for score in complexity.results.ordered_by_line():
            score.name = '%s.%s' % (node.name
 score.name)
            self.results.add(score)

    def highest_line_in_node(self
 node
 highest=0):
 ...
 node):
    ...
    def _tests_for_if(self
 if_node):
        try:
            return [test for test
 body in if_node.tests]
        except AttributeError:
            return [if_node.test]

    visitGenExprIf = visitListCompIf = visitIfExp = visitIf

    def __processDecisionPoint(self
 node):
        self.score += 1
        self.dispatch_children(node)
    ...
    def _visit_logical_operator(self
 node):
        self.dispatch_children(node)
...
class ModuleComplexity:
 ...
 code):
    ...
    def compute_complexity(self):
        ...
        self.add_module_results(complexity
 self.code)
        return complexity

    def add_module_results(self
 complexity
 code_or_node):
        end_line = max(1
 code_or_node.count('\n') + 1)
...
def show_complexity():
 ...
 SyntaxError):
    ...
    line_changes = compute_line_changes(old_complexities
 new_complexities)
    update_line_markers(line_changes)


def compute_scores_for(filename=None
 code=None):
    if filename:
...
def compute_line_changes(cached_complexities
 new_scores):
    changes = {}
    for line
 complexity in new_scores.iteritems():
        if complexity != cached_complexities.get(line
 None):
...
def compute_new_complexities(scores):
    new_scores = {}
    for score in scores:
        for line in range(score.start_line
 score.end_line + 1):
...
def update_line_markers(line_changes):
    ...
    for line
 complexity in line_changes.iteritems():
