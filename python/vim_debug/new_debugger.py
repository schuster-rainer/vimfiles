...
class Debugger:
    ...
    def start_py(self, fname):
        ...
        if which == 'Linux':
            ...
            sys.exit(1)
        pydbgp = os.path.join(vim_dir, 'python', 'pydbgp.py')

        subprocess.Popen((pydbgp, '-d', 'localhost:9000', fname),
                stdout=subprocess.PIPE, stderr=subprocess.PIPE, executable="c:\Python26\python.exe"  )
        self._type = 'python'
        return self.start()
